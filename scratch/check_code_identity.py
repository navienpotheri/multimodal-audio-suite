...
# Load generated codes from the run
# We can load the saved synthesized_speech.wav's codes, or just load them if we run generation again
# Let's run a quick greedy generation to get the exact generated codes
...
print(f"Comparing first {compare_len} steps:")
c0_match = sum(1 for i in range(compare_len) if c0_codes[i] == orig_codes[0, i].item())
c1_match = sum(1 for i in range(compare_len) if c1_codes[i] == orig_codes[1, i].item())

print(f"Codebook 0 exact match: {c0_match}/{compare_len} ({c0_match/compare_len*100:.2f}%)")
print(f"Codebook 1 exact match: {c1_match}/{compare_len} ({c1_match/compare_len*100:.2f}%)")
